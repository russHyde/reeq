...
#'   first two columns should be "feature_id" and "Length". "Length" should be a
#'   numeric column. The column names are
#'   taken to be the working ids for the samples (except feature_id and Length) and
...
#' feature_id, Length and a file.path. The file.path column contains the read count
#' for a given feature in that sample. Returns a data.frame with the count
#' column for each sample juxtaposed and with feature_id and Length columns.
#'
#' @param        count_list    List of data.frames. Each data.frame should have
#'   columns "feature_id", "Length" and a final column that contains the read
  ...
  if (!all(vapply(count_list, is_df_valid, logical(1)))) {
    stop(
      paste(
        ...
        "feature_id, Length and a count column (the latter can have any name but",
