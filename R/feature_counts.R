...
#'
#' Combines a list of data.frames into a single data.frame. The input
#' data.frames are of the format generated by featureCounts: the columns are
#' Geneid, Length and a file.path. The file.path column contains the read count
#' for a given feature in that sample. Returns a data.frame with the count
#' column for each sample juxtaposed and with Geneid and Length columns.
#'
#' @param        count_list    List of data.frames. Each data.frame should have
#'   columns "Geneid", "Length" and a final column that contains the read
#'   counts for some sample.
#'
#' @param        names_as_colnames   Boolean. If TRUE, the column ids in the
#' output dataframe are taken from the list names in `count_list`.
#'
#' @importFrom   magrittr      %>%
...
cbind_feature_counts <- function(
  ...
  # - Each data.frame should have a Geneid and a Length column

  # - The output should be a data.frame with Geneid, Length columns and then
  ...
  if (!all(vapply(count_list, is_df_valid, logical(1)))) {
    stop(
      paste(
        ...
        "Geneid, Length and a count column (the latter can have any name but",
  ...
  if (names_as_colnames) {
    count_list <- Map(
      function(df, cname) {
        stats::setNames(df, c("Geneid", "Length", cname))
      },
      count_list,
      names(count_list)
    )
